...
func main() {
	...
	if len(deathDeps) > 0 {
		...
		err = tombstone.Watch(ctx, graveyard, onDeathOfAny(deathDeps, func() {
			...
			if err != nil {
				log.G(context.TODO()).Printf("Error: failed to shutdown: %v\n", err)
			}
		}))
		if err != nil {
...
func onDeathOfAny(deathDeps []string, callback func()) tombstone.EventHandler {
	...
	return func(event fsnotify.Event) {
		...
		log.G(context.TODO()).WithField("tombstone", name).Info("recieved tombstone watch event")
		...
		if err != nil {
			log.G(context.TODO()).WithField("tombstone", name).Errorf("failed to read tombstone: %v", err)
			return
		}
