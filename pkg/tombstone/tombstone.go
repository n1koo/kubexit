...
func (t *Tombstone) String() string {
	...
	if err != nil {
		log.G(context.TODO()).Printf("Error: failed to marshal tombstone as json: %v\n", err)
		return "{}"
	}
...
func LoggingEventHandler(event fsnotify.Event) {
	if event.Op&fsnotify.Create == fsnotify.Create {
		log.G(context.TODO()).Printf("Tombstone Watch: file created: %s\n", event.Name)
	}
	if event.Op&fsnotify.Remove == fsnotify.Remove {
		log.G(context.TODO()).Printf("Tombstone Watch: file removed: %s\n", event.Name)
	}
	if event.Op&fsnotify.Write == fsnotify.Write {
		log.G(context.TODO()).Printf("Tombstone Watch: file modified: %s\n", event.Name)
	}
	if event.Op&fsnotify.Rename == fsnotify.Rename {
		log.G(context.TODO()).Printf("Tombstone Watch: file renamed: %s\n", event.Name)
	}
	if event.Op&fsnotify.Chmod == fsnotify.Chmod {
		log.G(context.TODO()).Printf("Tombstone Watch: file chmoded: %s\n", event.Name)
	}
}
